...
lastReviewedNote: "Reviewed for Team Logo Storage: local proof now pairs the focused pgTAP policy contract with real PNG/SVG Storage transport smoke."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Supabase Storage bucket or Team Logo object policy | `supabase db reset`; run `supabase/tests/20260805_full_schema_cutover.sql` and `supabase/tests/20260808_sys_files_team_logo_storage.sql` | through a temporary authenticated local user, upload, download, and remove one PNG and one SVG via the Storage API; verify a disallowed extension or unauthorized membership fails closed when the policy changes | Keep bucket configuration and `storage.objects` RLS in migrations. Prove the private bucket, MIME limits, exposed-schema absence of internal helpers, exact helper ACLs, owner/admin and new-team admission, member denial, referenced-object manager deletion, and uploader cleanup. Never use a service key for the transport smoke. |
